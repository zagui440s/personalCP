...
class Selected_pokemon(APIView):
    ...
    def get(self, request, id):  # <-- Notice id is now a parameter and its value is being pulled straight from our URL
        ...
        for move in range(len(serialized_pokemon[0]['fields']['moves'])):
                id = serialized_pokemon[0]['fields']['moves'][move]
                serialized_pokemon[0]['fields']['moves'][move] = json.loads(serialize('json', [Move.objects.get(id = id)]))[0]
